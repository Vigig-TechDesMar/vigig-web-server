...
{
    public static IEnumerable<T> BuildSearchResult<T>(IEnumerable<T> src,SearchUsingGet  request)
    {
        ...
        {
            Expression<Func<T, object>> orderExpr = p => ReflectionHelper.GetValueByName(typeof(T), request.SortField, p);
            src = request.Descending
                ? src.OrderByDescending(orderExpr.Compile()).ToList()
                : src.OrderBy(orderExpr.Compile()).ToList();
        }
